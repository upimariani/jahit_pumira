...
class mCustom extends CI_Model
{
    ...
    {
        $data['custom'] = $this->db->query("SELECT * FROM transaksi JOIN custom ON transaksi.id_transaksi = custom.id_transaksi JOIN kain ON custom.id_kain = kain.id_kain JOIN customer ON transaksi.id_customer=customer.id_customer where transaksi.id_transaksi= '" . $id . "'")->row();
        return $data;
    }
